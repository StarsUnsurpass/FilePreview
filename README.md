...
- **新增预览器**:
  ...
  - **证书 (Certificate) 预览**: 支持 `.cer`, `.crt`, `.der`, `.pem` 证书详情查看。
- **功能优化**:
  - **性能提升**: 针对 `TextPreviewer` 和 `HexPreviewer` 添加了大型文件截断逻辑（>1MB），确保预览秒开不卡顿。
  - **JSON 格式化**: `TextPreviewer` 现在会自动美化 JSON 文本内容。
  - **窗口定位**: 窗口现在会根据鼠标当前位置智能弹出，并自动适配多显示器工作区。
  - **复制增强**: 新增“复制文本内容”功能，支持一键从代码、证书或文件夹列表中提取文字。
  - **文件夹预览**: 升级为详情列表视图，显示子项大小、日期及顶级目录总大小统计。
- **技术债务**:
  - 升级至 `.NET 10`。
  - 引入 `Lazy<T>` 延迟加载预览器，优化内存占用及启动速度。
  - 彻底解决 WPF/WinForms 混合环境下的命名空间歧义冲突。

### 2026-01-07 (v1.2.0)
- **安装程序**: 引入 Inno Setup 安装脚本，支持一键构建安装包。
- **版本更新**: 升级版本号至 1.2.0。
